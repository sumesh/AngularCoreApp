...
export class PurchaseReturnListComponent implements OnInit {
  ...
  getData() {
    if (this.fblist.valid) {
      ...
      this.invservice.InvoiceDataService('api/purchasereturn/search/', this.fblist.value)
        .subscribe(data => {
          if (data) {
  ...
  setPageData() {
    ...
    localStorage.setItem('purchasereturnlist', JSON.stringify(pagedata));
  }

  getPageData() {
    var pagedata = JSON.parse(localStorage.getItem('purchasereturnlist'));
    if (pagedata) {
  ...
  redirectPage(page: string, obj: any) {
    if (page == "print") {
      this.setPageData();
      this.sessiondata.redirect('purchase/purchasereturn/view', { print: true, id: obj.SRID });
    }
    else if (page == "view") {
      this.setPageData();
      this.sessiondata.redirect('purchase/purchasereturn/view', { print: false, id: obj.SRID });
    }
    else if (page == "edit") {
      this.sessiondata.redirect('purchase/purchasereturn', { id: obj.SRID });
    }
    else if (page == "cancel") {

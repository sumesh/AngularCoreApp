...
@Component({
  ...
  templateUrl: './purchasereturnview.component.html',
  styleUrls: ['./purchasereturnview.component.scss']
})
export class PurchaseReturnViewComponent implements OnInit {
  ...
  getInvoiceData(invid: number, isprint: boolean) {
    this.invservice.InvoiceDataService('api/purchasereturn/view/', { IVID: invid, IsPrint: isprint })
      .subscribe(
        data => {
  ...
  openPrintAudit() {
    this.config.data.invid = this.invdata.SRID;

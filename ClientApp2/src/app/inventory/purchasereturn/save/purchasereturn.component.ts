...
export class PurchaseReturnComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userAccess = this.sessiondata.getUserAccess("PURCHASERETURN");
    this.createForm();
    this.ngOnChange();
    this.loadData();
    this.loadItem();
  ...
  loadData() {
    this.invservice.getInvoiceCommonData('api/purchasereturn/master/')
  ...
  loadItem() {
    ...
    this.invservice.InvoiceDataService('api/getitemlist/', { PageType: "PURCHASERETURN" })
      .subscribe(
        data => {
          if (data) {
            localStorage.setItem('purchasereturnitemlist', JSON.stringify(data));
          }
          else {
  ...
  createForm() {
    ...
    this.salesform = this.fb.group({
      ...
      sgstAmt: [{ value: 0, disabled: false }, Validators.compose([Validators.required, Validators.min(.1)])],
      cgstAmt: [{ value: 0, disabled: false }, Validators.compose([Validators.required, Validators.min(.1)])],
      igstAmt: [{ value: 0, disabled: false }, Validators.compose([Validators.required, Validators.min(.1)])],
  ...
  initItems() {
    return this.fb.group({
      ...
      price: [{ value: '', disabled: !this.userAccess.IsAdmin }, Validators.required],
  ...
  getInvoiceData() {
    ...
    && this.salesform.get("refFinyear").value != "0" && this.itemdatachange == 0) {
      ...
      this.invservice.InvoiceDataService('api/purchase/view/', {
...
this.TotalQty=0;
  ...
  setKFCessForGST(reggstno) {
    ...
    if (this.salesform.get("invtype").value == "LOCAL") {
      ...
      this.salesform.get("sgstAmt").setValidators([Validators.required, Validators.min(.1)]);
      this.salesform.get("cgstAmt").setValidators([Validators.required, Validators.min(.1)]);
    }
    else {
      ...
      this.salesform.get("igstAmt").setValidators([Validators.required, Validators.min(.1)]);
    }
  ...
  save(isValid: boolean, isPrint: boolean): void {
    ...
    if (isValid && confirm("Please confirm.")) {
      ...
      this.invservice.InvoiceDataService('api/purchasereturn/save/', salevalue)
        .subscribe(
          data => {
            ...
            if (data) {
              if (data['IsSuccess']) {
                ...
                if (isPrint) {
                  this.sessiondata.redirect("purchase/purchasereturn/view/", { print: true, id: data["RetID"] });
                }
                else {
                  this.reset();
                }

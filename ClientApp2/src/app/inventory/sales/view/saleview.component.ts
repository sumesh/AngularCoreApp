...
@Component({
  ...
  styleUrls: ['./saleview.component.scss']
})
export class SaleViewComponent implements OnInit {
  ...
  printWarrantyCondition: boolean = false;
  warrantyarray = [];
  displayedColumns: string[] = ['name', 'qty', 'weight', 'ucode', 'price', 'amount', 'discount', 'cgst', 'sgst', 'igst', 'kfcess', 'total'];
  ...
  getInvoiceData(invid: number, isprint: boolean) {
    this.invservice.InvoiceDataService('api/sale/view/', { IVID: invid, IsPrint: isprint })
      .subscribe(
        data => {
          ...
          if (data) {
            ...
            this.warrantyarray = this.invdata.WarrantyCondition.split('\n').filter(f=>f!="");
  ...
  updatePrintList(cnt): void {
    ...
    if (cnt == 0 && this.invdata.WarrantyCondition != '') {
      this.printWarrantyCondition = true;
    }
    else {
      this.printWarrantyCondition = false;
    }
  ...
  printWarrantyClick() {
    this.today = new Date().toString();
    setTimeout(() => { this.print(true); }, 1000);
  }

  print(isWarrenty: boolean): void {
    console.log(this.invdata.WarrantyCondition.split('\n'));
    ...
    printcss = `@media print { 
      ...
      .tax-table tr.items th,
      .taxtable tr.items td 
      {
       padding-top: 1px; 
      }
      .topalign
      ...
      .ul-10 li{margin-top:12px}
    } `;
    printContents = ''
    if (isWarrenty) {
      printContents = document.getElementById('print-section-warranty').innerHTML;
    }
    else {

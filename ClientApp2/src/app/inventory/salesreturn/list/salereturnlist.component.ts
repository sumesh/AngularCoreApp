...
@Component({
  ...
  templateUrl: './salereturnlist.component.html',
  styleUrls: ['./salereturnlist.component.scss']
})
export class SaleReturnListComponent implements OnInit {
  ...
  getData() {
    if (this.fblist.valid) {
      ...
      this.invservice.InvoiceDataService('api/salesreturn/search/', this.fblist.value)
        .subscribe(data => {
          if (data) {
  ...
  setPageData() {
    ...
    localStorage.setItem('salesreturnlist', JSON.stringify(pagedata));
  }

  getPageData() {
    var pagedata = JSON.parse(localStorage.getItem('salesreturnlist'));
    if (pagedata) {
  ...
  redirectPage(page: string, obj: any) {
    if (page == "print") {
      this.setPageData();
      this.sessiondata.redirect('sale/salesreturn/view', { print: true, id: obj.SRID });
    }
    else if (page == "view") {
      this.setPageData();
      this.sessiondata.redirect('sale/salesreturn/view', { print: false, id: obj.SRID });
    }
    else if (page == "edit") {
      this.sessiondata.redirect('sale/salesreturn', { id: obj.SRID });
    }
    else if (page == "cancel") {
